...
Elwa_stream_early <- Elwa_stream_daily %>% 
  filter(Date < as.Date("2017-07-01"))

Stream_daily.ts <- ts(Elwa_stream_early$Avg_Stage_Height, 
...
```{r}
ggplot(Elwa_sediment, aes(x = Date, y = Discharge)) + 
  geom_line() + 
  geom_smooth(method = 'lm')
...
sediment_daily.ts <- ts(Elwa_sediment$Discharge, start = c(2011, 09), frequency = 365)
